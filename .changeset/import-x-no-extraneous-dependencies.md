...
An import a package does not declare resolves only by accident of layout —
Node walks up to a parent `node_modules` and finds it there. That works
until the package moves, and it leaves any version range the dependency
declares unenforced for the importing package.
