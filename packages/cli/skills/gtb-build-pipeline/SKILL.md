...
**Codecov flag names.** `coverage:codecov:upload` names each flag after the package's unscoped `package.json` name (`@acme/utils` → `utils`), never the directory basename — a worktree or clone renames that, breaking Codecov's comparison of a flag against its own history. `codecov.yml` lists no flags: `flag_management.default_rules` applies per-package rules to whatever flags Codecov ingests, so a new package needs no config change. Two packages colliding after scope-stripping would silently share a flag, and nothing checks for that.
