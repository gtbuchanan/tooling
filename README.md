...
`carryforward: true` holds a package's last known coverage when it
doesn't upload, which turbo's cached upload leaf makes routine —
without it Codecov would read the missing flag as 0%. The cost is that
Codecov cannot tell coverage it carried forward from coverage that
never arrived, so a failed upload reports stale data until a later run
replaces it. `release.yml` re-runs CI on merge to main, which corrects
a one-off failure; re-run it if the stale number persists.
