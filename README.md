...
a per-package turbo task. Turbo caches uploads based on coverage content
— unchanged packages are skipped and Codecov carries forward their last
known data.

**Public repos:** works out of the box — no configuration needed.

**Private repos:** add a `CODECOV_TOKEN` repository secret, then pass it
